...
setup_suite() {
    ...
    docker-compose -f kine-compose.yml down -v
    docker-compose -f kine-compose.yml up -d
    return
...
setup() {
    ...
    assert "docker run --name=flannel-e2e-test-flannel1 -d --privileged $FLANNEL_DOCKER_IMAGE --etcd-endpoints=$etcd_endpt -v 10"

    # rm any old flannel container that maybe running, ignore error as it might not exist
    docker rm -f flannel-e2e-test-flannel2 >/dev/null 2>/dev/null
    assert "docker run --name=flannel-e2e-test-flannel2 -d --privileged $FLANNEL_DOCKER_IMAGE --etcd-endpoints=$etcd_endpt -v 10"
}

teardown() {
    echo "dumping subnets in etcd"
    docker run --rm -e ETCDCTL_API=3 $ETCDCTL_IMG etcdctl --endpoints=$etcd_endpt get --prefix /coreos.com/network/subnets 2>&1
